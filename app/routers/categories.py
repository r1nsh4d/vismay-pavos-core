...
async def create_category(
    ...
    existing = await db.scalar(
        select(Category).where(Category.tenant_id == payload.tenant_id, Category.name == payload.name)
    )
    if existing:
...
async def get_category(category_id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
...
async def delete_category(category_id: int, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    cat = await db.scalar(select(Category).where(Category.id == category_id))
    if not cat:
        raise AppException(status_code=404, detail="Category not found", error_code="NOT_FOUND")
    await db.delete(cat)
