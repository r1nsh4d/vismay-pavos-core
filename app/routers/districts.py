...
async def create_district(dist_in: DistrictCreate, db: AsyncSession = Depends(get_db)):
    dist = await district_svc.create_district(db, dist_in)
...
async def list_districts(state: str | None = None, db: AsyncSession = Depends(get_db)):
    districts = await district_svc.get_all_districts(db, state=state)
...
async def get_district(district_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    dist = await district_svc.get_district_by_id(db, district_id)
    if not dist:
...
async def update_district(district_id: uuid.UUID, dist_in: DistrictCreate, db: AsyncSession = Depends(get_db)):
    ...
    dist = await district_svc.update_district(db, dist, dist_in)
...
async def delete_district(district_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    await district_svc.delete_district(db, dist)
